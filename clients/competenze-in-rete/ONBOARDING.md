# Onboarding — Competenze in Rete (CIR)

> Documento per chi prende in gestione questo cliente per la prima volta. Ultimo aggiornamento: 2026-07-08 (Elisa Moratello).
> Fonti: pagina Notion cliente ["Comunicazione Competenze in Rete"](https://app.notion.com/p/muvilab/Comunicazione-Competenze-in-Rete-2c68360c6e1546299459c3b935ca8315) + `AGENTS.md` e `phases/05-reports/` di questa cartella.

---

## 1. Chi è il cliente

**Competenze in Rete (CIR)** affianca le aziende nella selezione di manager (middle-top management) e supporta i candidati nello sviluppo della loro carriera.

CIR è collegata a **NODUS HR**, che porta coaching e strumenti come *Insights Discovery* dentro ai team, per lo sviluppo manageriale.

> ⚠️ **Regola da non violare mai nei contenuti**: NODUS **non fa formazione** — non usare mai questa parola per NODUS. La distinzione CIR/NODUS va sempre tenuta chiara: CIR seleziona e sviluppa carriere, NODUS fa coaching.

C'è anche un terzo filone gestito nello stesso contenitore: **WINE** (WinePeople Talent), candidature nel settore vino/hospitality, con newsletter dedicata e candidature inviate da una referente esterna (Barbara).

**Target:**
- **B2B**: aziende medie (4-5M → 40-50M di fatturato; 15-20 → 200-250 dipendenti) che cercano figure di responsabilità (middle-top management) e sono disposte a spendere migliaia di euro per trovare la persona giusta.
- **B2C**: candidati che hanno bisogno di essere "attrattivi" sul mercato.

Cliente attivo dal 2023, rapporto tuttora in corso (il campo "Data Fine: 28/02/2025" su Notion è obsoleto/non aggiornato — ignoralo, il contratto attuale decorre da gennaio 2026).

---

## 2. Team

| Ruolo                           | Persona                   | Note                                              |
| ------------------------------- | ------------------------- | ------------------------------------------------- |
| Social Media Manager/newsletter | Elisa Moratello           | Riferimento principale operativo                  |
| Grafico                         | Enrico Grigolon           |                                                   |
| Video                           | Giovanni                  | Shooting esterni, content plan separato (vedi §7) |
| Web                             | Matteo Munaretto (Innove) | Esterno, referente sito competenzeinrete.net      |
| Account manager                 | Elena Caccia              |                                                   |

Non esiste un Web Designer interno assegnato — la gestione sito è esternalizzata a Innove.

---

## 3. Canali gestiti e cadenza (contratto da gennaio 2026)

**LinkedIn è il canale primario.** Facebook e Instagram sono cross-posting da LinkedIn (con gestione commenti mensile). Nessuna campagna a pagamento attiva — **Meta Ads non si fa per questo cliente**.

**Cadenza post:**
- 2 post/settimana, mix grafica + video
- Max 2 caroselli/mese (su LinkedIn: PDF/carosello normale se sono foto/backstage; su Meta: caroselli normali)
- 1 post/mese dedicato a NODUS ("rubrica" che racconta il legame NODUS-CIR)

**Contenuti WINE (annunci di lavoro):**

1. **Post candidature/grafiche**: il cliente pubblica per primo su LinkedIn → noi ripostiamo su Meta entro 2 giorni lavorativi (ci mandano email con grafica + copy già postato su LinkedIn). Su Instagram: storia con link alla candidatura sul sito.
2. 4. Tempistiche: 24-48 ore dalla ricezione

**Shooting video**: 4/anno (1/trimestre) con Giovanni

**Linguaggio**: sensibilità di genere richiesta — usare sempre "un/una" + carica (es. "un/una Sales Manager"), mai il solo maschile.

**Flusso PED**: invio intorno al 20 del mese via PostPickr con vista cliente e annotazioni dirette.

---

## 4. Newsletter (Mailchimp)

**3 invii/mese:**
1. **1 newsletter CIR redazionale** — target: aziende che cercano candidati. Contiene: un argomento del mese (già nel PED), grafica header, immagine coerente con il post social, selezione di post social, e 2 profili candidati (PDF caricati sul sito CIR, link nella newsletter).
2. **2 newsletter WINE** — solo candidature, formato copia/incolla. Le candidature arrivano da Barbara su Notion; si prepara la copertina su Canva (chiedere al grafico); farsi inviare i 4 profili del mese a inizio mese; le date di uscita si concordano con Barbara, che invia le candidature 7 giorni prima.

**Tag Mailchimp**: NL CIR → `newsletterCIR` + `CIR` · NL WINE → `newsletterWINE` + `WINE` (richiedere la grafica 7 giorni prima).

**File contatti condiviso col cliente** (da aggiungere al database Mailchimp): [Google Sheet](https://docs.google.com/spreadsheets/d/1SdDFEC3-e8MGjMYN_6C6FbSdF2JqgU607HKQaKy548w/edit).

**Nota operativa**: la revisione strutturale della newsletter (test su posizionamento CTA, formato, ecc.) **non è inclusa nel contratto attuale** — le ottimizzazioni fatte finora sono un'iniziativa in più concordata internamente (Elena Caccia / Elisa Moratello), non richiesta dal cliente. Tienilo presente se il cliente dovesse notare/chiedere di questi cambi.

**Problema aperto**: bounce rate ricorrente di 13-22 contatti a invio su più campagne consecutive (marzo-giugno 2026) — segnalato ma non ancora risolto. Vedi raccomandazioni nei report di marzo/aprile.


## 5. White paper

4/anno: 2 target candidati + 2 target aziende. Per ognuno:
1. Creare la guida sul sito (accesso: vedi password manager, non più su Notion in chiaro)
2. Valutare di dedicare la NL redazionale del mese al lancio
3. Creare un Google Sheet collegato al sito per raccogliere i contatti

I "vecchi" white paper (Temporary Manager, Clima Aziendale) vanno ripubblicati periodicamente per generare lead — quello sul Temporary era stato messo in pausa fino a giugno 2026, verificare se è stato riattivato.

---

## 6. Temi editoriali 2026 (must-have, da calendario cliente)

- **HR Fractional Manager / Temporary Manager** — tema ricorrente, molto performante (vedi §9)
- **Passaggio generazionale** — in collaborazione con Drapelli Partners, collegabile a temporary manager e HR fractional. Strategia contenuti approvata: https://app.notion.com/p/muvilab/Strategia-contenuti-3586e1358be180f0b7fdefa73ff3866b?source=copy_link
- **Clima aziendale** — tema trasversale da toccare da più angolazioni; case study/interviste previste giu-lug 2026, pubblicazione set-ott 2026
- **Comunicazione interna** — possibile nuovo servizio da sviluppare (house organ, eventi interni, social network interno)
- **Case study e interviste** — coinvolgere clienti e fractional manager per testimonianze (es. progetto Iras)
- **Candidati in vetrina** — sperimentazione di post singoli (invece di post aggregati), avviata da aprile 2026
- **Collaborazioni future** — sinergie possibili con Francesco Bernardi (Blumantra) su employer branding

**Eventi/fiere già coperti nel 2026**: meeting Insights a Milano (8 marzo), Global Summit HR come espositori (22-23 aprile), Vinitaly (comunicato in parallelo ad aprile).

### Wishlist con budget extra (non finanziato oggi)
- Google Ads su "clima aziendale" (autunno 2026, valutare)
- Notorietà geografica: Treviso, Pordenone, Friuli
- Newsletter trimestrale generica per candidati (consigli di carriera, cultura aziendale, job board) — in fase di preventivo con Elena Caccia, obiettivo riattivare il database e traffico al sito

---

## 7. Dove trovare le cose

- **Pagina cliente Notion**: [Comunicazione Competenze in Rete](https://app.notion.com/p/muvilab/Comunicazione-Competenze-in-Rete-2c68360c6e1546299459c3b935ca8315) — contratto operativo, credenziali (ora su password manager), risorse, temi editoriali. È la fonte primaria per tutto ciò che riguarda l'operatività quotidiana.
- **Content plan video (Giovanni)**: pagina Notion collegata "Competenze in Rete - Content Plan" (linkata dalla pagina cliente).
- **Report social**: pagine Notion "Dati report social <mese> 2026" (marzo, aprile, maggio-giugno finora) — dati grezzi con grafici Meta/LinkedIn.


